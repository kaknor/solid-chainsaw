  ...
  , root_dir_(root_dir)
{
  ...
  int opt = 1;
  if (setsockopt(listener_fd_, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
		 &opt, sizeof(int)) < 0)
    std::cerr << "An error occured while setting options to the socket" << std::endl;
  ...
  if (bind(listener_fd_, (struct sockaddr*) &address, sizeof (struct sockaddr_in)) < 0)
  ...
  int accepter_fd_ = accept(listener_fd_, (struct sockaddr*) &address, &address_len);
  if (accepter_fd_ < 0)
